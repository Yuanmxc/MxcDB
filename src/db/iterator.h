#ifndef MXCDB_ITERATOR_H_
#define MXCDB_ITERATOR_H_
...
public:
  TweLevelIterator(std::shared_ptr<Iterator> &index_iter_,
      ...
      : block_function_(block_function), arg_(arg), options_(options),
        index_iter(index_iter_), data_iter() {}
...
public:
  Merageitor(std::shared_ptr<Iterator> *children, int n)
      : children_(new IteratorWrapper[n]), n_(n), current_(nullptr),
        direction_(kForward) {
    for (int i = 0; i < n; i++) {
...
NewMergingIterator(std::shared_ptr<Iterator> *children, int n) {
  if (n == 0) {
    return nullptr;
  } else if (n == 1) {
    return children[0];
