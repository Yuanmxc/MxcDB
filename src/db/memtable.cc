...
State Tablebuilder::Finish() {
  ...
  if (state.ok() && filter_block != nullptr) { // Write filter block
    ...
    mlog->debug("write filter_block block form offset {} size {}",
                filter_block_handle.offset(), filter_block_handle.size());
  }
  if (state.ok() && filter_block != nullptr) { // Write metaindex block
    ...
    mlog->debug("write meta_index block form offset {} size {}",
  ...
  if (state.ok()) {
    ...
    mlog->debug("write index_block block form offset {} size {}",
